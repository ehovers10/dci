...
In the remainder of this paper, I examine two extant proposals in the literature and contend that neither captures these observations in their entirety. My preferred analysis is based upon the collaborative update framework, supplementing it with an analysis of {% gloss bare plurals %}.
...
Before we get to my preferred account, it is worth exploring another approach to discourses such as [Bears](#bears-ex){: #bears-show .tooled} that does give pride of linguistic place to the corrective element of the completion utterance.
...
In [Movie](#movie){: #movie-show .tooled}, the initiation carries a packet of information. In the responses, we alternately have denial of an implicature (a), a presupposition (b), and a choice of words (c). In each case, the rest of the packet remains untouched, and may even be available for anaphoric reference later on.
...
<!-- DRT -->
{% def Standard DRT %}
  {{ "pragmatics|drs" | formalize }}
  <hr>
  {{ "pragmatics|contributions" | formalize }}
{% enddef %}
...
<!-- Spanish ldrt -->
{% ex Spanish semantics %}
  {{ "spanish" | example: "ldrt" }}
{% endex %}

## Strictness, shifts, and structure

The general collaborative update framework forms the backbone of my preferred analysis of [Bears](#bears-ex){: #bears-show .tooled} But the specifics of the discourse dynamics are determined by the semantic structure of the particular utterance involved. In this case, the the key elements are the *bare plural* subjects of the utterances. I think these are best Bare plurals are, in this sense, variably strict.
...
Unfortunately, [Thinning](#thinning-def){: #thinning-show .tooled} brings the strict analysis up against a sticky situation. Consider:
...
Despite its de-sticking success, Thony Gillies {% cite gillies2007 | noname %} has argued that the standard response is not the preferred extraction method. We can preserve the strict analysis of counterfactuals if we allow them to act as monsters {% cite kaplan1977 | noname %}, shifting the index of evaluation in the course of interpretation. In particular, if the set determined by the consequent of the conditional is evaluated against a context that is fixed in part by the set determined by the antecedent, then the extraction is achieved.
...
Most English speakers take [Hibernation](#hibernating-bears-ex){: #hibernating-bears-show .tooled} to be true, even when told that a fair number of bears don't hibernate.[^hibernation] This poses a problem for the strict analysis of bare plurals.
