...
I contend that the conjecture/correction discourse pair provides a genuine contribution to inquiry. In the previous chapter, I motivated this status by providing a model of discourse dynamics that carves out a distinctive operation on information that this contribution makes. In this chapter I explore another criterion for genuine a contribution: that it be represented in some conventionalized linguistic tool. I think that this tool is {% include appendices/concept.html word="focus," %} and that the virtues of collaborative discourse dynamics come to prominence in explaining focus interpretation when we examine {% include appendices/concept.html word="contrastive topic" %} specifically.

The following little discourse, which I label *Bears*, will be the stalking horse for the discussion to follow.

<!-- Grizzly bears -->
{% include examples/ex2.html term="grizzly" %}

The bracketed material represents {% include appendices/concept.html word="focus." base="" %} *Grizzly* in the (b) sentence is marked with a *cf* for {% include appendices/concept.html word="contrastive focus." base="contrast" e="" %} We will have more to say about the importance of focus in this dialogue later on. For now, there are a number of interesting pretheoretical observations to be made about the [Bears] dialogue. 

First, the (b) utterance is not merely a dismissal of (a). Instead, it is a natural follow up to, which is to say that (b) {% include appendices/concept.html word="coheres" base="coherence" e="" %} with (a). An ability to resolve a coherence relation among a group of utterances in a discourse is a crucial element of interpreting the discourse in context. A particularly salient coherence relation may force an interpretation to light. For instance, consider the following example due to Andrew Kehler {% include appendices/ref.html id="kehler2000" o="" p="539" e=":" %}
  
<!-- Presidents -->
{% include examples/ex2.html term="presidents" %}
...
<!-- Druggy bears -->
{% include examples/ex2.html term="druggy" %}
...
<!-- Boars -->
{% include examples/ex2.html term="boars" %}

In [Boars](#boars), the second utterance breaks the flow of inquiry to attempt to fix a perceived error in the preceding discourse. In this example, the disputative *You mean...* is used to signal an attempted {% include appendices/concept.html word="repair strategy." base="" %} 

The observation that the [Bears](#bears) dialogue exhibits coherent discourse suggests that the disputative tone of its completion ought not to be treated as signaling a break in the inquiry. Imposing the refinement relation on the discourse eliminates any sense of fault on the part of the initiation in allowing for proper uptake. Instead, we interpret the completion utterance as a {% include appendices/concept.html word="correction," base="" e="" %} which is characterized by a dual function of providing: (i) a *denial* of some element of the preceding attempted contribution to discourse and (ii) a *positive contribution* that is related in a salient way to the preceding attempted contribution. 

<!-- Correction -->
{% include examples/ex.html type="obs" term="correction" defn=site.data.definitions.obs.correction %}

As elements of coherent dialogue involving correction, I refer to sentences such as the initiation in [Bears](#bears) as {% include appendices/concept.html word="correctibles." base="correctible" %}

While the completion in the [Bears](#bears) dialogue is a natural one, it certainly isn't the only {% include appendices/concept.html word="felicitous" base="felicity" e="" %} response to the initiation. It is open to the responder to challenge the initiation in another way, but interestingly, it is also open to the initiator to retrench in certain ways, as in the following alternate extension of the *Bears* initiation:

<!-- Teddy bears -->  
{% include examples/ex2.html term="teddy" %}
...
<!-- Keys -->
{% include examples/ex2.html term="keys" %}
...
<!-- Resiliance -->
{% include examples/ex.html type="obs" term="resiliance" defn=site.data.definitions.obs.resiliance %}
...
<!-- Productivity -->
{% include examples/ex.html type="obs" term="productivity" defn=site.data.definitions.obs.productivity %}
...
<!-- Susceptibility -->
{% include examples/ex.html type="obs" term="susceptibility" sent="Example" sub="Sub-example" defn=site.data.definitions.obs.susceptibility %}
...
In the remainder of this paper, I examine two extant proposals in the literature and contend that neither captures these observations in their entirety. My preferred analysis is based upon the collaborative update framework, supplementing it with an analysis of {% include appendices/concept.html word="bare plurals." base="" e="" %}

# Correction as contrastive topic

A salient feature of the [Bears](#bears) dialogue is the presence of {% include appendices/concept.html word="focus" %} on *grizzly* in the completion. Focus is a lingusitic tool whose use serves primarily to *package* information carried by other components. For instance, focus can be be used to divide the informational content of an utterance into that which is assumed to be part of the common ground and that which is newly contributed {% include appendices/ref.html id="jackendoff1972" o="n" e="." %} The virtue of information packaging is in aiding uptake for the other participants in the discourse by showing them how the contribution is intended to fit with what has preceeded. 
...
One of the most influential accounts of focus interpretation is the *alternative semantics* developed by Mats Rooth {% include appendices/ref.html id="rooth1985,rooth1992" o="" e="." %} The idea behind alternative semantics is that focus-sensitive constructions have two semantic values: an *ordinary* semantic value ( {% include examples/sem.html term='&#8729;' index="o" %}) and a *focus* semantic value ( {% include examples/sem.html term='&#8729;' index="f" %}). The ordinary semantic value is whatever linguistic theory minus a theory of focus comes up with. And the focus semantic value for an expression is a set of *alternatives* to the ordinary semantic value. That is, the focus semantic value collects together all elements from the domain of discourse that are type-identical to the focused element of a phrase. More formally:

<!-- Focus semantic values -->
{% capture focussem %}
{% include examples/eqn2.html term="focus" type="sem" %}
{% endcapture %}
{% include examples/ex.html type="def" term="Focus semantic value" sent="Example" sub="Sub-example" defn=focussem %}
...
<!-- Cookie sem -->
{% include examples/ex2.html term="cookie" type="sem" %}
...
<!-- QA-congruence -->
{% capture qacong %}
{% include examples/eqn2.html term="qa-congruence" type="sem" %}
{% endcapture %}
{% include examples/ex.html type="def" term="QA-Congruence" sent="Example" sub="Sub-example" defn=qacong %}
...
By whatever path the discourse participants make their way, they are constantly coordinating their contributions around the *QUD*. To be relevant, an assertion must provide at least a partial answer to this question {% include appendices/ref.html id="roberts1996" o="n" p="21"%}. Roberts takes questions to have the standard, set of propositions, semantic value, which provides for a natural, set-theoretic account of the partial-answerhood relation. A partial answer to a question *q* is a proposition that entails, for at least one member proposition *q*<sub>i</sub> of the interpretation of the question, either that *q*<sub>i</sub> is true or that it is false. If the dynamical function of asserting is to eliminate certain propositions from the common ground, then the *QUD* framework requires that felicitous assertions eliminate along the contours of the *QUD*. An assertion whose content cross-cuts the partition of the common ground imposed by the QUD fails to provide a relevant contribution to discourse.

The *QUD* model can easily incorporate everything above regarding QA-congruence and focus interpreation. But, not all conversations begin with an explicit question. Many start with an assertion, and seem roll along fruitfully from there. In Roberts' framework, the discourse structure is a theoretical posit --  a set of organizing principles that unify a set of apparently disparate data points drawn from speaker intuitions about semantic content and pragmatic felicity. As such, *QUDs* needen't be drawn from actual utterances in the discourse; it is perfectly possible for them to be implicit in the conversation. While every appropriate assertion must bring us closer to an answer to some question, discourse participants frequently {% include appendices/concept.html word="accommodate" base="" e="" %} the question necessary to make sense of an assertion in context.
...
<!-- Picnic -->
{% include examples/ex2.html term="picnic" %}
...
Contrastive focus bears properties that are systematically different from standard focus. In particular, it is realized in English by a distinctive intonation pattern, labeled the B-accent to standard focus' A-accent {% include appendices/ref.html id="buering2003" o="n" p="512" e="." %} But the general function of highlighting some element from within a selected set of alternatives seems to be exhibited by focus in all its forms. The key, then, is to locate an antecedent in prior discourse to which the semantic value of the contrastively focus ed element conforms.
...
<!-- Picnic structure -->
{% include examples/ex2.html term="picnic" type="extra" %}
...
The mechanism of accommodation is only loosely understood, but Richmond Thomason's {% include appendices/ref.html id="thomason1990" o="" p="343" %} account in terms of obstacle elimination gives us something to grab on to:
...
The completion utterance is distinctively *disputative*. It seems to simultaneously *deny* the original statement and *substitute* an alternative to it. Jennifer Spenader and Emar Maier {% include appendices/ref.html id="spenader2009" e="," %} expanding on work of Maier and Bart Geurts {% include appendices/ref.html id="geurts1998a,geurtsa" e="," %} attempt to treat corrections as *downdates* on the information state. On this proposal, the function of corrections is to remove from the discourse representation something that was previous added to it. A wrinkle is that corrections do not eliminate information wholesale. Consider, for example:
...
<!-- Movie -->
{% include examples/ex2.html term="movie" %}
...
{% endcapture %}
{% include examples/ex.html type="def" term="DRS" sent="Example" sub="Sub-example" defn=drsstuff %}
...
{% endcapture %}
{% include examples/ex.html type="def" term="Downdate" sent="Example" sub="Sub-example" defn=dd %}
...
<!-- Managed -->
{% include examples/ex2.html term="managed" %}
...
<!-- Managed -->
{% include examples/ex2.html term="managed" type="sem" %}
...
<!-- Madrigals -->
{% include examples/ex2.html term="madrigals" %}
...
<!-- Madrigals -->
{% include examples/ex2.html term="madrigals" type="ldrt" %}
...
<!-- Spanish -->
{% include examples/ex2.html term="spanish" %}
...
<!-- Spanish ldrt -->
{% include examples/ex2.html term="spanish" type="ldrt" %}
...
{% endcapture %}
{% include examples/ex.html type="def" term="Strict analyses" sent="Example" sub="Sub-example" defn=strict %}
...
<!-- Thinning -->
{% capture thin %}
{% include examples/eqn2.html genus="counterfactual conditional" term="thinning" type="sem" %}
{% endcapture %}
{% include examples/ex.html type="def" term="Thinning" sent="Example" sub="Sub-example" defn=thin %}

Unfortunately, [Thinning](#thinning) brings the strict analysis up against a sticky situation. Consider:
  
<!-- Honey -->
{% include examples/ex2.html term="honey" %}
...
{% endcapture %}
{% include examples/ex.html type="def" term="Variable strict" sent="Example" sub="Sub-example" defn=variably %}
...
{% endcapture %}
{% include examples/ex.html type="def" term="Shifty strict" sent="Example" sub="Sub-example" defn=shifty %}
...
<!-- Hibernation -->
{% include examples/ex2.html term="hibernate" %}
...
<!-- Cuddly bears -->
{% include examples/ex2.html term="cuddly" %}
...
{% endcapture %}
{% include examples/ex.html type="def" term="Generic" sent="Example" sub="Sub-example" defn=generic %}
...
{% endcapture %}
{% include examples/ex.html type="def" term="Structural" sent="Example" sub="Sub-example" defn=generic %}
...
<!-- Homogeneity -->
{% include examples/ex.html type="def" term="Homogeneity" sent="Example" sub="Sub-example" defn=site.data.definitions.homogeneity.def %}
...
<!-- Playing bears -->
{% include examples/ex2.html term="playing" %}
...
> "A speaker who chooses a sentence involving GEN rather than one of the overt quantifiers signals that it is presupposed that the cases in the domain of quantification are uniform with respect to the property attributed by the scope of the quantifier." {% include appendices/ref.html id="vonfintel1997" o="n" p="34" %}

[^schwarzs]: See also Roger Schwarzschild's discussion of distributive contexts for plurals. For Schwarzschild, homogeneity is lexically encoded by the distributivity operator as applied to plurals and falls out of his understanding of plurals in terms of quantification over parts of pluralities combined with a four-valued semantics in which presupposition failure is understood as identity between the positive and negative extensions of a sentence {% include appendices/ref.html id="schwarzschild1994" o="" p="222" e="." %}
...
<!-- Polar bears -->
{% include examples/ex2.html term="polar" %}
...
{% include examples/eqn2.html genus="collaborative update" term="initiation" type="sem" %}

##### Continuation utterance
{% include examples/eqn2.html genus="collaborative update" term="continuation" type="sem" %}

##### Completion utterance
{% include examples/eqn2.html genus="collaborative update" term="completion" type="sem" %}

{% endcapture %}
{% include examples/ex.html type="def" term="General collaborative update" sent="Example" sub="Sub-example" defn=gencu %}
...
A {% include appendices/concept.html word="database" base="" e="" %} is a collection of data structured by a series of {% include appendices/concept.html word="tables" base="table" e="" %}
that represent interrelations among the points of data. The information contained in a database can be manipulated in a variety of ways principal among which is the {% include appendices/concept.html word="query," base="" e="" %} which pulls specific information from one or more tables, packaging it into a {% include appendices/concept.html word="view" base="" e="" %}, itself a table in form, but a mere image of the underlying database. Views allow the user to access and make use of data without impacting the underlying structure of the database. 

A {% include appendices/concept.html word="relational algebra" base="" e="" %} extends standard set theoretic operations with a set of operations on tables and provides a means of representing manipulations of structured data in a neat way {% include appendices/ref.html id="codd1970" o="n" e="." %} For the purposes of representing the discourse evolution of disputative dialogue involving plural predication, we add a subset of the relational algebra framework to collaborative update. Namely, we co-opt operations of: {% include appendices/concept.html word="projection" base="" e="," %} {% include appendices/concept.html word="outer join" base="" e="," %} {% include appendices/concept.html word="grouping" base="group" e="," %} {% include appendices/concept.html word="counting" base="count" e="," %} and {% include appendices/concept.html word="renaming" base="rename" e="." %}
