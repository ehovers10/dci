...
The structural analysis is strict in that it demands universal quantification over the entire set determined by the bare plural. The perceived looseness of bare plurals is captured by ensuring that the set determined by the bare plural is an element of the plan of inquiry space determined by the paritions imposed on the common ground.

As with the story about shifty strict counterfactuals, the real work comes in getting clear about how the context evolves in response to bare plural utterances. And it is here that the framework of collaborative update semantics earns its weight.

## Pragmatics of plurals and partitions

Plurals carry a *homogeneity presupposition* {% include ref.html id="schwarzschild1994,vonfintel1997" o="n" e="." %} In order for sentences involving them to be interpretable, it is assumed that the members of the set determined by the plural are uniform with respect to whether or not they satisfy the predicate. In the framework of collaborative update semantics, a homogeneity presupposition is captured by the presence of a default, trivial partitioning of the common ground, which is itself catalogued by a relation in i<sub>**R**</sub>. If no such partition is present, it is accommodated. The sentence is then evaluated as a universal quantifier against this partition as outlined in [Structurally universal quantifier](#structurally-universal-quantifier).
...
The presence of contrastive focus in the completion of [Bears](#bears) signifies the denial of the homogeneity presupposition. The content of the focused element dictates the way in which a substitiute for homogeneity is made.

Informally, the process goes like this: Utterance of a bare plural sentence is just like every other in that it initiates a sandbox and presents a proposition for consideration. The homogeneity presupposition represents the saturation of the propositional radical that is the plural sentence. If the presupposition is accommodated and the proposal accepted, the completion closes the sandbox and merges the result into the information state.

But if the presupposition is unacceptable (and the line of inquiry is accepted) an interlocutor can correct the utterance by offering an alternative saturation of the propositional radical. The contrastive focus serves to indicate the corrective role of the completion utterance. The substitution is enacted by adding a partition to the common ground that carves the focus set out from the common ground set. If there is no further discussion, the sandbox is closed and the modified, repartitioned information state is merged.

More formally, the information state is updated like this:

| As are B | &rArr; (+ sandbox, present plurals with default partition)

+ This gets us out of a sticky situation...
+ The simple semantics is preferable...
+ The proposal captures the observations...
